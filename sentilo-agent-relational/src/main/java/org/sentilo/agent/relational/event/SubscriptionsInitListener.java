...
/**
 * Este proceso solo se ejecuta una vez: cuando la aplicación arranca. Se encarga de inicializar/registrar las subscripciones definidas en el 
 * fichero de propiedades y a las cuales debe estar subscrito este agente.  
 * Tiene definida la restricción de que el registro no se lleve a cabo en el entorno de desarrollo, para asi evitar conflictos con el entorno local
 * y las estaciones de trabajo de los desarrolladores.
 */
@Component
@Profile("dev")
public class SubscriptionsInitListener implements org.springframework.context.ApplicationListener<org.springframework.context.event.ContextRefreshedEvent>{
	...
	void subscribe(){
		// Este proceso lee las subscripciones definidas en el fichero subscription.properties y las agrupa por Ds 
		// a utilizar para definir para persistir los datos. Para cada grupo define un MessageListener que se subscribirá
		...
		if(!CollectionUtils.isEmpty(subscriptionsDef)){
			...
			Iterator<String> subscriptionsKeys = subscriptionsDef.stringPropertyNames().iterator();
			Map<String,List<String>> dsGroup = new HashMap<String, List<String>>();
			
			while(subscriptionsKeys.hasNext()){
				String subscription = subscriptionsKeys.next();
	...
	private void registerMessageListeners(Map<String,List<String>> dsGroup){
		// Para cada Ds definido, definimos un grupo de subscripciones, y para cada grupo tendremos un messageListener. Cada ML estara registrado a todas 
		// las subscripciones del grupo.
		Iterator<String> dsKeys = dsGroup.keySet().iterator();
		while(dsKeys.hasNext()){
			String dsName = dsKeys.next();
			List<String> dsSubscriptions = dsGroup.get(dsName);
			MessageListener messageListener = new MessageListenerImpl(dsName, dataTrackService);
			for(String dsSubscription : dsSubscriptions){
	...
	private void processSubscriptionDef(String subscription, Map<String,List<String>> dsGroup){
		String[] dataSourcesNames = subscriptionsDef.getProperty(subscription).split(",");
		...
		for(String dataSourceName : dataSourcesNames){								
			...
			List<String> dsSubscriptions = dsGroup.get(dataSourceName);
	...
	public void setDataTrackService(DataTrackService dataTrackService) {
