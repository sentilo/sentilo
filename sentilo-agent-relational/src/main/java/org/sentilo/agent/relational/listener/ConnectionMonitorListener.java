...
public class ConnectionMonitorListener implements SmartLifecycle {
  ...
  private String monitorChannelName = MONITOR_CHANNEL;
  private String monitorMessageListenerName;
  ...
  public ConnectionMonitorListener(final String monitorChannelName, final String monitorMessageListenerName) {
    this();
    this.monitorChannelName = monitorChannelName;
    this.monitorMessageListenerName = monitorMessageListenerName;
  }

  @Override
  public void start() {
    if (listenerContainer.isActive() && listenerContainer.isRunning()) {
      monitorListener = (StringUtils.hasText(monitorMessageListenerName) ? new MonitorMessageListenerImpl(monitorMessageListenerName) : new MonitorMessageListenerImpl());
      monitorTopic = new PatternTopic(monitorChannelName);
      running = true;
