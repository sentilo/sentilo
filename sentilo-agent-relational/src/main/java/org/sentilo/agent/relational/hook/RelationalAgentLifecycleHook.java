 ...
 */
package org.sentilo.agent.relational.hook;
...
@Component
public class RelationalAgentLifecycleHook extends AgentLifecycleHook {

  private static final Logger LOGGER = LoggerFactory.getLogger(RelationalAgentLifecycleHook.class);

  @Autowired
  private AgentRelationalRepository repository;
  ...
  protected void doGracefulShutdown() {
    LOGGER.debug("Call relational agent doGracefulShutdown method.");

    // ... and flush pending data
    ((AgentRelationalBatchRepositoryImpl) repository).flush();
