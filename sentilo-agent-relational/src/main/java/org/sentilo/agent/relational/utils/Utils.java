...
public abstract class Utils {
	...
	/**
	 * Valida el format de la subscripcio. És a dir, valida que tingui un format vàlid per a la PSAB, i.e., que el format sigui
	 *    subscription_type: resourceid1:resourceid2
	 * on 
	 * <ul>
	 * <li> subscription_type ha de ser data, alarm o order</li>
	 * <li> resourceid1 i resourceid2 són opcionals i permeten afitar més a quin recurs és vol fer la subscripció</li>
	 * </ul>  
	 * @param subscription
	 * @return
	 */
	public static boolean isValidSubscription(String subscription){
		...
		if(StringUtils.hasText(subscription)){
			try{
				String[] tokens = subscription.split(Constants.TOPIC_TOKEN);
