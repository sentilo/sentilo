...
public class PlatformIntegration {
  ...
  public void validateGetObservations() {
    ...
    assertTrue("There was an error processing request to platform", StringUtils.hasText(response));
  }
  ...
  public void setInvalidTokenProgrammatically() {
    ...
    final String uri = "data/provider35";
    final RequestContext rc = new RequestContext(uri);
    rc.setIdentityToken(invalidToken);
    final String response = httpRestClient.get(rc);
    assertTrue("There was an error processing request to platform", StringUtils.hasText(response));
  }
