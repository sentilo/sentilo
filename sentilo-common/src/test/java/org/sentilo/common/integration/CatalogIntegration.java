...
public class CatalogIntegration {
	...
	protected void init(){
		ClassPathXmlApplicationContext context = new ClassPathXmlApplicationContext("classpath:spring/catalog-integration-context.xml");
		httpRestClient =context.getBean(RESTClient.class);
	}
	
	public void validateApiPermissions() {
		String uri = "api/permissions";
		String response = httpRestClient.get(uri);
		//Un ejemplo de respuesta es {"permissions":[{"source":"prov1","target":"prov1","type":"WRITE"},{"source":"prov2","target":"prov2","type":"WRITE"}]}		
	...
	public void validateApiAuth(){
		String uri = "api/credentials";
		String response = httpRestClient.get(uri);
		//Un ejemplo de respuesta es {"authorizations":[{"entity":"app1","token":"token3"},{"entity":"app2","token":"token4"},{"entity":"app3","token":"token5"}]}		
		System.out.println(response);
		assertTrue("No se ha realizado correctamente la llamada al catalogo",StringUtils.hasText(response));
	...
	public static void main(String[] args){
		CatalogIntegration driver = new CatalogIntegration();
