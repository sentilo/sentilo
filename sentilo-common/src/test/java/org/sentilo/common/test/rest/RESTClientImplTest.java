...
public class RESTClientImplTest {
	...
	public void invalidGet() throws Exception{
		String path = "data";
		StatusLine statusLine = new BasicStatusLine(HttpVersion.HTTP_1_0, HttpStatus.SC_FORBIDDEN, "Forbidden request");
	...
	public void get() throws Exception{
		String path = "data";
		String responseContent="Lorem ipsum";
		StatusLine statusLine = new BasicStatusLine(HttpVersion.HTTP_1_0, HttpStatus.SC_OK, "");
		
		when(httpClient.execute(notNull(HttpGet.class))).thenReturn(httpResponse);
		when(httpResponse.getStatusLine()).thenReturn(statusLine);
		when(httpResponse.getEntity()).thenReturn(new StringEntity(responseContent));
		
		String result = restClient.get(path);
