 ...
 */
package org.sentilo.agent.historian.event;
...
public class SubscriptionsInitListener extends AbstractSubscriptionsInitListener {
  ...
  public void subscribe() {
    ...
    LOGGER.info("Initializing historian agent subscriptions");
    if (!CollectionUtils.isEmpty(subscriptionsDef) && StringUtils.hasText(subscriptionsDef.getProperty("subscriptions"))) {
      final String subscriptionTopics = subscriptionsDef.getProperty("subscriptions");

      LOGGER.debug("Found {} subscriptions to register", subscriptionTopics.split(",").length);

      for (final String topic : subscriptionTopics.split(",")) {
        registerSubscription(messageListener, Utils.buildTopic(topic));
      }

    } else {
      LOGGER.info("No subscriptions found to register");
    }
  }
}
