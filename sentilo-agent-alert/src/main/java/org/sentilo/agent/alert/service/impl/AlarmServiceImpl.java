...
public class AlarmServiceImpl  implements AlarmService, ApplicationListener<CheckFrozenAlarmEvent> {
	...
	private Map<String, MessageListenerImpl> currentListeners = new HashMap<String, MessageListenerImpl>();
	...
	public void loadAndSubscribeToInternalAlarms(){
		// Las subscripciones a registrar vienen dadas por las alarmas internas definidas en el catalogo de la PSAB.
		// Por lo tanto, el primer paso es recuperar del catálogo todas las alarmas de tipo interno: esta petición la haremos 
		// accediendo directamente al repositorio del catálogo. 
		// Despues para cada una de las alarmas, registraremos un nuevo listener asociado a los datos del sensor asociado a la alarmas.
		logger.info("Initializing alarm agent subscriptions");
		List<Alarm> internalAlarms = getInternalAlarms();
		
		// Si no se hace ningún registro, el agente no permanecerá a la escucha y se parará. Si lo que queremos es que esté siempre
		// alerta, deberíamos subscribirlo almenos a un canal (por ejemplo uno mock y de esta manera forzar que siempre esté escuchando)
		
		// Tendremos un listener por cada sensor al cual debamos estar subscritos. Este listener deberá procesar las N alarmas
		// asociadas a los datos de ese sensor.
		Map<String, MessageListenerImpl> listeners = new HashMap<String, MessageListenerImpl>();
		
		if(!CollectionUtils.isEmpty(internalAlarms)){
			logger.debug("Found {} internal alarms to process", internalAlarms.size());
			
			for(Alarm alarm : internalAlarms){
				// El identificador unívoco de cada listener vendrá definido por el nombre del canal (topic) al cual estará asociado.
				// Y toda alarma interna tiene asociada un proveedor y un sensor. Por lo tanto, tenemos un listener asociado a cada 
				// uno de los sensores para los cuales hay alarmas. Este listener se encarga de evaluar, cada vez que llega un dato,
				// las diferentes alaarmas asociadas al sensor.
				String topicToListen = generateTopicName(alarm);
	 ...
	 */
	public void onApplicationEvent(CheckFrozenAlarmEvent event) {
	...
	public void updateAndRegisterListeners(Map<String,MessageListenerImpl> listeners){
		// Para cada listener definido en listeners vemos si ya forma parte de la colección de listeners existentes.
		// 1. Si existe, actualizamos u lista de alarmas
		// 2. Si no existe, lo añadimos y registramos en el listenerContainer
		// Por último, si alguno de los listeners ya existentes no forma parte de la nueva lista lo eliminamos tanto de la colección currentListeners
		...
		for(String listenerName: listeners.keySet()){
			if(currentListeners.containsKey(listenerName)){
				...
				MessageListenerImpl currentListener = listeners.get(listenerName);
		...
		for(String listenerName: currentListeners.keySet()){			
	...
	private void removeInvalidListeners(List<String> listenersToRemove){
		for(String listenerName: listenersToRemove){						
	...
	private List<Alarm> getInternalAlarms() {
		...
		Criteria queryCriteria = Criteria.where("type").is(AlarmType.INTERNAL.name());								
		List<Alarm> content = mongoOps.find(new Query(queryCriteria), Alarm.class);
		logger.debug("... and found {} alarms to process.", content.size());
		return content;
	}
	
	private String generateTopicName(Alarm alarm){		
		return AlertUtils.buildDataTopicAssociateToAlarm(alarm);
	}
		
			
	private void registerSubscriptionIntoContainer(MessageListener messageListener, String topic){		
