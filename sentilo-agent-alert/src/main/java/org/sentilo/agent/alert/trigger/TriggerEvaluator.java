...
public class TriggerEvaluator {
  ...
  public TriggerResult evaluate(final InternalAlert alert, final String value) {
    ...
    LOGGER.debug("Evaluating alert {} with sensor value {}", alert.getId(), value);
    ...
    try {
      ...
      // This exception should never occurs because the format of an alert expression is validated
      // when
    ...
    LOGGER.debug("Alert rule evaluation result was {}", result.triggerConditionChecked());
