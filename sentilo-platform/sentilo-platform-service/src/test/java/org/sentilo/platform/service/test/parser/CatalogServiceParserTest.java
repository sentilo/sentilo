...
public class CatalogServiceParserTest {
	...
	public void parsePermissions() throws Exception {							
		String json = "{\"permissions\":[{\"source\":\"prov1\",\"target\":\"prov1\",\"type\":\"WRITE\"},{\"source\":\"prov2\",\"target\":\"prov2\",\"type\":\"WRITE\"}]}";											
		
		PermissionsMessage message = parser.parsePermissions(json);		
		assertTrue(message.getPermissions()!=null && message.getPermissions().size() == 2);
	}
	
	@Test
	public void parseCredentials() throws Exception {																			
		String json = "{\"credentials\":[{\"entity\":\"app1\",\"token\":\"token3\"},{\"entity\":\"app2\",\"token\":\"token4\"},{\"entity\":\"app3\",\"token\":\"token5\"},{\"entity\":\"prov1\",\"token\":\"token1\"}]}";
		CredentialsMessage message = parser.parseCredentials(json);		
		assertTrue(message.getCredentials()!=null && message.getCredentials().size() == 4);
	}	
	
	@Test
	public void parseCatalogResponse() throws Exception {							
		String json = "{\"code\":\"200\",\"errorMessage\":\"\"}";											
		
		CatalogResponseMessage message = parser.parseCatalogResponse(json);
	...
	public void parseAlarmsOwners() throws Exception{
		String json = "{\"alerts\":[{\"alertId\":\"alerta1\",\"ownerEntityId\":\"app_demo_provider\"},{\"alertId\":\"alerta2\",\"ownerEntityId\":\"sentilo-catalog\"}]}";		
		
		AlertsOwnersResponseMessage message = parser.parseAlertsOwners(json);
		assertNotNull(message.getAlerts());
		assertEquals(2,message.getAlerts().size());		
	}
