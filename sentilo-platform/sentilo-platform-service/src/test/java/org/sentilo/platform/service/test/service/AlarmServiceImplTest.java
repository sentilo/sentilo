...
public class AlarmServiceImplTest {
  ...
  public void setAlarm() {
    ...
    when(sequenceUtils.getAid(ALERT_ID)).thenReturn(Optional.of(23l));
    doNothing().when(resourceService).checkAlertState(ALERT_ID);
  ...
  public void setAlarmFromDisabledAlert() {
    when(message.getMessage()).thenReturn("Alarm event");
    when(message.getAlertId()).thenReturn(ALERT_ID);
    doThrow(ResourceOfflineException.class).when(resourceService).checkAlertState(ALERT_ID);
  ...
  public void setAlarmFromUnknowAlert() {
    when(message.getMessage()).thenReturn("Alarm event");
    when(message.getAlertId()).thenReturn(ALERT_ID);
    doThrow(ResourceNotFoundException.class).when(resourceService).checkAlertState(ALERT_ID);
  ...
  public void getLastAlertAlarms() {
    ...
    when(message.getAlertId()).thenReturn(ALERT_ID);
    when(sequenceUtils.getAid(notNull(String.class))).thenReturn(Optional.of(1l));
  ...
  public void getUnknownAlertOwner() {
    when(resourceService.getAlert(ALERT_ID)).thenReturn(Optional.empty());
  ...
  public void getAlertOwner() {
    ...
    when(resourceService.getAlert(ALERT_ID)).thenReturn(Optional.of(alert));
