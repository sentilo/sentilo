...
public class AlarmServiceImplTest {
	...
	public void setAlarm(){
		...
		when(message.getAlarmId()).thenReturn("alarm1");
		
		String channel = ChannelUtils.buildTopic(PubSubChannelPrefix.alarm, message.getAlarmId()).getTopic();
	...
	public void getLastMessagesFromAlarm(){
		String alarm = "alarm1";		
		Set<String> amids = buildAmids();
				
		when(message.getAlarmId()).thenReturn(alarm);
		...
		verify(message, times(2)).getAlarmId();					
