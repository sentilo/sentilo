...
public class AlarmServiceImpl extends AbstractPlatformServiceImpl implements AlarmService {
  ...
  public String getAlertOwner(final String alertId) {
    final Optional<String> alertOwner = getOwnerFromCache(alertId);
    if (!alertOwner.isPresent()) {
      throw new ResourceNotFoundException(alertId, "Alert");
    }
    return alertOwner.get();
  }
  ...
  public List<Alarm> getLastAlarms(final AlarmInputMessage message) {
    ...
    final Optional<Long> aid = sequenceUtils.getAid(message.getAlertId());
    if (aid.isPresent()) {
      messages.addAll(getLastAlarms(aid.get(), message));
    }

    return messages;
  }

  private Optional<String> getOwnerFromCache(final String alertId) {
    if (alertsOwners.get(alertId) == null) {
      final Optional<Alert> alert = resourceService.getAlert(alertId);
      if (alert.isPresent()) {
        alertsOwners.put(alertId, alert.get().getEntity());
      }
    }

    return Optional.ofNullable(alertsOwners.get(alertId));
  }
  ...
  private void checkTargetResourceState(final AlarmInputMessage message) {
    resourceService.checkAlertState(message.getAlertId());
  }

  private Long registerAlarmMessage(final AlarmInputMessage message) {
    final Optional<Long> aid = sequenceUtils.getAid(message.getAlertId());
    final Long amid = sequenceUtils.getNewAmid();
    ...
    fields.put(AID, Long.toString(aid.get()));
    ...
    sRedisTemplate.zAdd(keysBuilder.getAlertAlarmsKey(aid.get()), timestamp, amid.toString());
