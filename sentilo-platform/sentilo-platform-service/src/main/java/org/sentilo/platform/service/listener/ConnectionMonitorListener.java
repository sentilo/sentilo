...
/**
 * El objetivo de esta clase es comprobar que la conexion que utiliza el listenerContainer, para mantenerse subscrito a los datos, esta Ok.
 * Para ello lo que se hace es subscribirse al canal MONITOR al arrancar la plataforma y enviar periodicamente un dato asi como comprobar el estado de la 
 * conexión de subscripcion. Si en cualquier momento se detecta un problema en esta conexion, se reinicia el listener. 
 */
@Component
public class ConnectionMonitorListener implements SmartLifecycle {
	...
	private final String MONITOR_CHANNEL = "MONITOR:PSAB*";
	...
	private MessageListenerImpl monitorListener = null;	
	...
	public void validateConnection(){
		...
		if(isRunning()){		
			try{
				// Es importante tener en cuenta que la conexion de subscripcion solo permite invocar a un cjto reducido de comandos de Redis.
				// Por lo que simplemente nos subscribimos a un canal, y, si todo va bien, nos desubscribimos.
				subscribeMonitor();
				removeMonitorSubscription();
				logger.info("Subscribe connection is OK!");
			}catch(RedisConnectionFailureException e){
				// Si se produce cualquier problema en el proceso de subscripcion/desubscripcion lo que hacemos es lo siguiente:
				// 1. Parar/cancelar el thread de subscripcion que utiliza el container para que deseche la conexión a Redis subyacente que internamente  
				//    utiliza ya que probablemente sea invalida y de aqui el error. Esto lo hacemos invocando al metodo stop del listenerContainer
				// 2. Reiniciamos la conexión a Redis del container: simplemente se trata de volver a invocar al metodo lazyListen. Y este metodo se 
				//    puede invocar mediante el registro de un listener cuando el listener no esta escuchando (i.e., lo que hace la 
				//    llamada subscribeMonitor). También se puede hacer invocando al metodo start del listenerContainer. 						
	...
	@Override
	public void stop() {
		// Eliminar subscripcion a canal MONITOR
		if(isRunning()){			
			try {
				listenerContainer.removeMessageListener(monitorListener, monitorTopic);
