 ...
 * 
 * As not spring-data-redis 1.0.3 implements all redis available commands (e.g. zremrangebyrank ) and
 * RedisMessageListenerContainer no lets unsubscribe for a subscription (see http://forum.springsource.org/showthread.php?133937-Spring-Data-Redis-pub-sub-unsubscribe), 
 * all the functionality needed is replicated in this project.  
 * 
 * Upgrade: new versions of spring-data-redis already implements the unsubscribe functionality (see https://jira.springsource.org/browse/DATAREDIS-107)
 * 
...
public class JedisTemplate<K, V> {
	...
	// Merece la pena utilizar este template? Internamente realiza diversos pasos  (pipeline, transaccionalidad, .... ) que para nosotros no son necesarios
	...
	public <T> T execute(JedisCallback<T> action) {
		...
		Jedis conn = jedisPoolUtils.getResource();
	...
	public Set<String> keys(final String pattern){		
		return execute(new JedisCallback<Set<String>>() {			
			public Set<String> doInRedis(Jedis connection) {
	...
	public String get(final String key){		
		return execute(new JedisCallback<String>() {			
			public String doInRedis(Jedis connection) {
	...
	public Map<String, String> hGetAll(final String key){
		return execute(new JedisCallback<Map<String, String>>() {			
			public Map<String, String> doInRedis(Jedis connection) {
