...
public class NotificationRetryRepositoryImpl implements NotificationRetryRepository {
  ...
  public List<NotificationRetryEvent> getEventsToRetry(final long currentTimestamp, final long limit) {
    LOGGER.debug("Querying up {} retry notification events with lowest scores", limit);
    final Set<TypedTuple<String>> events = redisTemplate.opsForZSet().popMin(SORTED_SET_KEY, limit);

    // final Set<String> events = redisTemplate.opsForZSet().rangeByScore(SORTED_SET_KEY, 0,
    // currentTimestamp, 0, limit);
     ...
     */
    for (final TypedTuple<String> tt : events) {
      notificationEvents.add(eventParser.unmarshall(tt.getValue()));
    }
  ...
  public Long getTotalEventsToRetry(final long currentTimestamp) {
    LOGGER.debug("Querying total notification events to retry with a delay time (or score) lower than {}", currentTimestamp);
