...
public class NotificationDeliveryServiceImpl implements NotificationDeliveryService {
  ...
  public void retryNotifications() {
    if (retryNotificationsEnabled) {
      ...
      final List<NotificationRetryEvent> eventsToRetry = repository.getEventsToRetry();
      for (final NotificationRetryEvent notificationRetryEvent : eventsToRetry) {
        notificationRetryEvent.incrementRetryCount();
        pushNotification(notificationRetryEvent);
      }
      LOGGER.debug("Finished retry notifications process. Running time (millis) = {}", System.currentTimeMillis() - initTs);
    }
  }
  ...
  protected void pushNotification(final NotificationRetryEvent notificationRetryEvent) {
    ...
    try {
      ...
      restClient.setHost(notificationParams.getEndpoint());
      // Previous secret key must be overridden at every call to force that every notification
      // uses the assigned secret key to the channel's subscription.
      restClient.setSecretKey(notificationParams.getSecretCallbackKey());
      LOGGER.debug("Push notification {} to endpoint: {}", notificationRetryEvent.getMessage(), notificationParams.getEndpoint());
      restClient.post("", notificationRetryEvent.getMessage());
