...
public class BodyLengthInterceptor implements SentiloRequestHandlerInterceptor {
  ...
  @Value("${sentilo.server.api.payload.max_length:0}")
  ...
  private boolean apply(final SentiloRequest request) {
    return !request.getEntitySource().equals(serviceConfig.getConfigValue("sentilo.master.application.id", SentiloConstants.DEFAULT_CATALOG_ID));
  }
