...
public class PlatformJsonMessageConverter extends BaseJsonMessageConverter{
	...
	protected void writeInternal(Object o, SentiloResponse response) throws IOException , JsonConverterException{				
		try {			
			ByteArrayOutputStream out = super.writeInternal(o);
			response.setBody(out, DEFAULT_CONTENT_TYPE);
		}catch (MessageNotWritableException ex) {
			throw new JsonConverterException("Could not write JSON: " + ex.getMessage(), ex, false);
		}
	}
	
	protected Object readInternal(Class<?> clazz, SentiloRequest request) throws JsonConverterException{		
		try{
	...
	public Long parseTimestamp(String timestamp) throws JsonConverterException{									
		try {			
	...
	public Date parseDate(String date) throws JsonConverterException{				
		try {			
	...
	public Integer parseInteger(String integer) throws JsonConverterException {				
		try {
