...
public class SubscribeHandler extends AbstractHandler {
	...
	private RequestMessageValidator<SubscribeInputMessage> validator = new SubscribeValidator();
	...
	public void onDelete(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		// DEL /subscribe/eventType/resourceId con resourceId pudiendo ser compuesto por 2 ids (caso provider+sensor)
				
		validateResourceNumberParts(request, 0, 3);						
		Subscription subscription = parser.parseBasicRequest(request);			
		validator.validateRequestMessageOnDelete(new SubscribeInputMessage(subscription));		
		// En este caso no hace falta validar la autorizacion ya que el remove solo se aplica sobre las subscripciones de quien hace la peticion
	...
	public void onGet(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		// En este caso no hace falta validar la autorizacion ya que el get solo retorna subscripciones de quien hace la peticion
		List<Subscription> subscriptions = subscribeService.get(subscription);		
	...
	public void onPut(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		Subscription subscription = parser.parseRequest(request, SubscribeType.DATA);		
		...
		//Validamos que quien hace la peticion de subscripcion tiene permiso de lectura sobre la entidad propietaria del recurso (data, order o alarm)				
	...
	public void setSubscribeParser(SubscribeParser parser) {
		this.parser = parser;		
	}
}
