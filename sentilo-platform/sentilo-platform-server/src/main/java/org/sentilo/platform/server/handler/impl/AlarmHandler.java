...
public class AlarmHandler extends AbstractHandler {
	...
	private RequestMessageValidator<AlarmInputMessage> validator = new AlarmValidator();
	...
	public void onGet(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		AlarmInputMessage inputMessage = parser.parseGetRequest(request);
		validator.validateRequestMessageOnGet(inputMessage);	
		String alarmOwner = alarmService.getAlertOwner(inputMessage.getAlarmId());
		validateReadAccess(request.getEntitySource(), alarmOwner);
		List<Alarm> lastAlarmsMessages = alarmService.getLastMessages(inputMessage);
	...
	public void onPut(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		// PUT /alarm/idAlarm con un mensaje en el body de manera opcional
		
		validateResourceNumberParts(request, 1, 1);
		AlarmInputMessage inputMessage = parser.parseRequest(request);
		validator.validateRequestMessageOnPut(inputMessage);
		// La alarma pertenece a un proveedor o a una app cliente.
		// Recuperamos el propietario de la alarma y validamos que el solicitante de la accion tiene autorizacion para escribir sobre los recursos del propietario
		String alarmOwner = alarmService.getAlertOwner(inputMessage.getAlarmId());		
	...
	public void setAlarmParser(AlarmParser parser) {
		this.parser = parser;		
	}	
}
