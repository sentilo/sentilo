...
public class DataHandler extends AbstractHandler {
	...
	private RequestMessageValidator<DataInputMessage> validator = new DataValidator();
	...
	public void onDelete(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		DataInputMessage inputMessage = parser.parseDeleteRequest(request);
	...
	public void onGet(SentiloRequest request, SentiloResponse response) throws PlatformException {		
		...
		validateResourceNumberParts(request, 1, 2);
		DataInputMessage inputMessage = parser.parseGetRequest(request);
		validator.validateRequestMessageOnGet(inputMessage);							
		validateReadAccess(request.getEntitySource(), inputMessage.getProviderId());
		List<Observation> lastObservations = dataService.getLastObservations(inputMessage);
	...
	public void onPut(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		DataInputMessage inputMessage = parser.parsePutRequest(request);
