...
public class RequestListenerThread extends Thread {
  ...
  @Value("${sentilo.server.api.socket.port:8081}")
  private int port;
  @Value("${sentilo.server.api.socket.tcp-backlog:50}")
  private int socketTcpBacklog;
  @Value("${sentilo.server.api.socket.timeout-ms:5000}")
  private int socketMillisecondsTimeout;
  @Value("${sentilo.server.api.socket.buffer-size:8192}")
  private int socketBufferSize;
  @Value("${sentilo.server.api.socket.stale-connection-check:false}")
  private boolean staleConnectionCheck;
  @Value("${sentilo.server.api.socket.tcp-nodelay:true}")
  private boolean tcpNoDelay;
  @Value("${sentilo.server.api.origin-server}")
  private String originServer;
  @Value("${sentilo.server.api.handler.pattern:*}")
  ...
  private void manageConnection(final DefaultHttpServerConnection conn) throws IOException, InterruptedException {
    ...
    try {
      ...
      // LOGGER.warn("An error has been detected while waiting to new requests on port {}", port);
    }
