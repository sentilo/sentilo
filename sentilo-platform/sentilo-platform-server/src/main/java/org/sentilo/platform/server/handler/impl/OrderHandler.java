...
public class OrderHandler extends AbstractHandler {
	...
	private RequestMessageValidator<OrderInputMessage> validator = new OrderValidator();
	...
	public void onGet(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		OrderInputMessage inputMessage = parser.parseGetRequest(request);
		validator.validateRequestMessageOnGet(inputMessage);							
		validateReadAccess(request.getEntitySource(), inputMessage.getProviderId());
		List<Order> lastOrders = orderService.getLastOrders(inputMessage);
	...
	public void onPut(SentiloRequest request, SentiloResponse response) throws PlatformException {
		...
		validateResourceNumberParts(request, 1, 2);
		OrderInputMessage inputMessage = parser.parseRequest(request);
	...
	public void setOrderParser(OrderParser parser) {
		this.parser = parser;		
	}	
}
