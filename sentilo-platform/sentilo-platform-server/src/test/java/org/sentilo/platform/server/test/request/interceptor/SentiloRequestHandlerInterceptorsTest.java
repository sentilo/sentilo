...
import org.sentilo.platform.common.security.service.AuthenticationService;
...
import org.sentilo.platform.server.exception.PlatformUnavailableException;
...
import org.sentilo.platform.server.request.interceptor.StatusInterceptor;
...
public class SentiloRequestHandlerInterceptorsTest {
  ...
  @Mock
  private SentiloModuleConfigService serviceConfig;
  ...
  public void pass_bodyLengthInterceptor() {
    ...
    when(serviceConfig.getConfigValue("sentilo.master.application.id", SentiloConstants.DEFAULT_CATALOG_ID))
        .thenReturn(SentiloConstants.DEFAULT_CATALOG_ID);
  ...
  public void pass_rateLimiterInterceptor() {
    ...
    ReflectionTestUtils.setField(srhi, "publishService", publishService);
    final String provider = "mockProvider";
  ...
  public void error_when_rateLimiterInterceptor() {
    ...
    verify(rateLimiterService).allow(provider);
    verify(publishService).publishInboundRateLimiterAlarm(provider, QuotaContextHolder.getContext(QuotaContext.Type.ENTITY));
  }
  ...
  public void error_when_instance_rateLimiterInterceptor() {
    ...
    verify(publishService).publishInboundRateLimiterAlarm(provider, QuotaContextHolder.getContext(QuotaContext.Type.GLOBAL));
  }

  @Test
  public void pass_statusInterceptor() {
    final StatusInterceptor srhi = new StatusInterceptor();
    ReflectionTestUtils.setField(srhi, "entityMetadataRepository", entityMetadataRepository);
    ReflectionTestUtils.setField(srhi, "authorizationService", authorizationService);

    when(entityMetadataRepository.hasIdentitiesLoaded()).thenReturn(true);
    when(authorizationService.hasPermissionsLoaded()).thenReturn(true);

    srhi.invoke(sRequest);
  }

  @Test(expected = PlatformUnavailableException.class)
  public void error_when_statusInterceptor() {
    final StatusInterceptor srhi = new StatusInterceptor();
    ReflectionTestUtils.setField(srhi, "entityMetadataRepository", entityMetadataRepository);
    ReflectionTestUtils.setField(srhi, "authorizationService", authorizationService);

    when(entityMetadataRepository.hasIdentitiesLoaded()).thenReturn(false);
