...
public class ResourceOwnerContextTest {
  ...
  @Mock
  private EntityMetadataMessage message;

  @Before
  public void setUp() {
    MockitoAnnotations.initMocks(this);
    when(message.getEntity()).thenReturn(entityId);
    when(message.getToken()).thenReturn(token);
    when(message.getTenantId()).thenReturn(tenantId);
  }

  @Test
  public void buildContext() {
    final ResourceOwnerContext context = new ResourceOwnerContext(message);
    ...
    Assert.assertEquals(message, context.getMetadata());
  }
}
