...
@Profile("!cluster")
public class InMemorySlidingWindowServiceImpl extends AbstractSlidingWindowServiceImpl implements SlidingWindowService {

  private static final String KEY_SEPARATOR = ":";

  private final Map<String, ConcurrentMap<String, AccountBucket>> windowsAccountsBuckets =
      new HashMap<String, ConcurrentMap<String, AccountBucket>>();
  private final Map<String, WindowAccountInfo> windowsCounters = new HashMap<String, WindowAccountInfo>();
  private Lock lock;
  ...
  public RateLimiterStatus getAccountsStatus() {
    ...
    accountsCountersCopy.forEach(entry -> {
        if (entry.hasBuckets()) {
          rlState.addAccountStatus(entry.getAccount(), buildAccountState(entry.getQuota(), entry.getTotal(), entry.getOldestBucket().getCreatedAt()));
        }
      });
   // @formatter:on
  ...
  private long getAccountWaitingTime(final String account) {
    ...
    return minutesToReset(oldestBucketTime);
  }
  ...
  private ConcurrentMap<String, AccountBucket> buildWindowBucket() {
    // @formatter:off
    final ConcurrentMap<String, AccountBucket> windowBuckets = ExpiringMap.builder()
        .expiration(DURATION, TimeUnit.SECONDS)
        .expirationPolicy(ExpirationPolicy.CREATED)
        .expirationListener((final String bucketKey, final AccountBucket bucket) ->
          {
  ...
  class WindowAccountInfo {
    ...
    private long total;
    private long quota;
    private final List<AccountBucket> orderedBuckets;
    ...
    public void removeBucket(final String bucketLabel, final long bucketCount) {
      ...
      // FIFO collection). Therefore, removeBucket always removes the first bucket because it is the
      // oldest.
      orderedBuckets.remove(0);
      total -= bucketCount;
    }
