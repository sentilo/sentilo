...
public class EntityMetadataRepositoryImpl implements EntityMetadataRepository {
  ...
  private boolean identitiesLoaded = false;

  private final Lock lock = new ReentrantLock();

  @Override
  public boolean hasIdentitiesLoaded() {
    return identitiesLoaded;
  }
  ...
  public void loadEntitiesMetadata() {
    try {
      ...
      if (entitiesMetadata != null && !CollectionUtils.isEmpty(entitiesMetadata.getEntitiesMetadata())) {
        for (final EntityMetadataMessage entityMetadata : entitiesMetadata.getEntitiesMetadata()) {
          ...
          if (entityMetadata.isActive()) {
            // Filter inactive entities so that their tokens cannot be used to authenticate requests
      ...
      // Retry now if identities has not been loaded yet.
      if (!identitiesLoaded) {
