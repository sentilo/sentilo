...
public class DefaultAlarmServiceOperationsImplTest {
  ...
  @InjectMocks
  private DefaultAlarmServiceOperationsImpl service;
  ...
  @Test
  public void getLastAlarmMessages() {
    service.getLastAlarmMessages(message);

    verify(restClient).get(any(RequestContext.class));
    verify(converter).unmarshal(any(String.class), eq(AlarmsOutputMessage.class));
  }
}
