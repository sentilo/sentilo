...
public class AlarmMessageConverterTest {
	...
	public void buildAlarmBody() throws MessageNotWritableException{		
		AlarmInputMessage message = new AlarmInputMessage(ALARM_ID,ALARM_MESSAGE);
		String json = converter.marshall(message);
		String body = "{\"message\":\"umbral superado\"}";
		assertNotNull(json);			
	...
	public void unmarshallAlarmMessages() throws MessageNotReadableException{		
		String json = "{\"messages\":[{\"message\":\"límite superior superado\",\"timestamp\":\"21/02/2013T17:49:24\",\"sender\":\"appDemo\"},{\"message\":\"límite inferior superado\",\"timestamp\":\"22/02/2013T17:49:24\",\"sender\":\"appDemo\"}]}";
		AlarmsOutputMessage message = converter.unmarshall(json);
		assertNotNull(message);
		assertEquals(2, message.getMessages().size());
		assertEquals("appDemo", message.getMessages().get(0).getSender());
