...
public class DataServiceOperationsIntegrationTest {
	...
	public void _01_sendSimpleObservation() throws Exception{
		DataInputMessage message = new DataInputMessage("testApp", "sensor1", "23");
	...
	public void _02_sendForbiddenSimpleObservation() throws Exception{
		DataInputMessage message = new DataInputMessage("testDemo", "sensor1", "23");						
		boolean error = false;
		try {
			platformTemplate.getDataOps().sendObservations(message);	
	...
	public void _03_sendSensorObservations() throws Exception{
		DataInputMessage message = new DataInputMessage("testApp","sensor1");
		SensorObservations sensorObservations = new SensorObservations("sensor1");
	...
	public void _04_sendProviderObservations() throws Exception{
		DataInputMessage message = new DataInputMessage("testApp");
		ProviderObservations providerObservations = new ProviderObservations();
	...
	public void _05_getLastObservationFromSensor() throws Exception{
		DataInputMessage message = new DataInputMessage("testApp","sensor1");		
		ObservationsOutputMessage response = platformTemplate.getDataOps().getLastObservations(message);					
	...
	public void _06_getLastObservationsFromSensor() throws Exception{
		...
		DataInputMessage message = new DataInputMessage("testApp","sensor1",filterParams);
	...
	public void _11_getLastObservationsFromProvider() throws Exception{
		...
		DataInputMessage message = new DataInputMessage("testApp",filterParams);
		
		ObservationsOutputMessage response = platformTemplate.getDataOps().getLastObservations(message);					
		assertTrue(response!=null && !CollectionUtils.isEmpty(response.getSensors()));
		//La lista contiene almenos dos registros, uno para cada sensor del proveedor que hemos utilizado en el test
		assertTrue(response.getSensors().size()>=2); 
		//Y para cada sensor tenemos almenos 2 observaciones, las utilizadas en este test --> un minimo de 4 observaciones en total
	...
	public void _14_getLastObservationsFromProviderInPeriod() throws Exception{
		...
		//La lista contiene almenos dos registros, uno para cada sensor del proveedor que hemos utilizado en el test
		assertTrue(response.getSensors().size()>=2); 
		//Y para cada sensor tenemos almenos 2 observaciones que entran dentro del periodo indicado, las utilizadas en este test --> un minimo de 4 observaciones en total
		assertTrue(countObservations(response.getSensors())>4);
	}
	
	@Test
	public void _15_removeLastObservationsFromSensor() throws Exception{
		//El NUM_MAXIM_OBSERVATIONS simplemente es para indicar que retorne todas las observaciones posibles
	...
	public void _16_removeLastObservationsFromProvider() throws Exception{
		//El NUM_MAXIM_OBSERVATIONS simplemente es para indicar que retorne todas las observaciones posibles
		final QueryFilterParams filterParams = new QueryFilterParams(SentiloConstants.NUM_MAXIM_ELEMENTS);
		DataInputMessage message = new DataInputMessage("testApp",filterParams); //El NUM_MAXIM_OBSERVATIONS simplemente es para indicar que retorne todas las observaciones posibles		
	...
	private int countObservations(List<SensorObservations> sensors){
		int count = 0;
		
		for(SensorObservations sensorObs : sensors){
	...
	private List<Observation> getObservations(){
		...
		Observation obs1 = new Observation("1", System.currentTimeMillis());
		Observation obs2 = new Observation("2", System.currentTimeMillis()-3000);
	...
	private List<SensorObservations> getSensorObservations(){
		List<SensorObservations> sensorObservations = new ArrayList<SensorObservations>();
		SensorObservations sensorObs1 = new SensorObservations("sensor1", "45.3 56.76");
		sensorObs1.setObservations(getObservations());
		SensorObservations sensorObs2 = new SensorObservations("sensor2");
