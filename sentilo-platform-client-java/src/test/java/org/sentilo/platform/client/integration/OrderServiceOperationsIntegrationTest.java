...
public class OrderServiceOperationsIntegrationTest {
	...
	public void _01_publish() throws Exception{
		OrderInputMessage message = new OrderInputMessage("testApp", "sensor1", new OrderMessage("stop"));
	...
	public void _02_publishWithoutOrderMessage() throws Exception{
		OrderInputMessage message = new OrderInputMessage("testApp");
		boolean error = false;
		try {
			platformTemplate.getOrderOps().publish(message);
	...
	public void _03_publishWithoutPermission() throws Exception{
		OrderInputMessage message = new OrderInputMessage("appDemo", new OrderMessage("stop"));
		boolean error = false;
		try {
			platformTemplate.getOrderOps().publish(message);
		} catch (PlatformClientAccessException e) {
	...
	public void _04_getLastOrdersFromSensor() throws Exception{
		OrderInputMessage message = new OrderInputMessage("testApp","sensor1");		
		OrdersOutputMessage response = platformTemplate.getOrderOps().getLastOrders(message);					
