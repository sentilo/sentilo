...
public class ComponentTypeServiceImplTest {
  ...
  public void getComponentsTypeFromProvider() {
    ...
    final List<String> typeIdsList = Arrays.asList(componentType1.getId(), componentType3.getId());
    ...
    when(mongoOps.getCollectionName(any(Class.class))).thenReturn(componentCollectionName);
    when(mongoOps.getCollection(componentCollectionName)).thenReturn(dbCollection);
    when(dbCollection.distinct(any(String.class), any(DBObject.class))).thenReturn(typeIdsList);
    when(mongoOps.find(any(Query.class), any(Class.class))).thenReturn(expectedComponentTypesList);

    final List<ComponentType> componentTypesList = service.findComponentTypesByProvider(providerId);

    verify(mongoOps).getCollectionName(any(Class.class));
    verify(mongoOps).getCollection(any(String.class));
    verify(mongoOps).find(any(Query.class), any(Class.class));
