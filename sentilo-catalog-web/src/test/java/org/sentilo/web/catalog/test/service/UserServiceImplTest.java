...
public class UserServiceImplTest extends AbstractBaseCrudServiceImplTest {
  ...
  @Test
  public void addUsersToSector() {
    final String sectorId = "mock_sector";
    final String[] usersIds = {"user_1", "user_2"};

    service.addUsersToSector(sectorId, usersIds);

    verify(mongoOps).updateMulti(argThat(new ParamInMatcher("userName", usersIds)), any(Update.class), eq(User.class));
  }

  @Test
  public void removeUsersFromSector() {
    final String sectorId = "mock_sector";
    final String[] usersIds = {"user_1", "user_2"};
