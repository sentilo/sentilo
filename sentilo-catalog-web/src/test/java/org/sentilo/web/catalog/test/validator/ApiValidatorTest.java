...
public class ApiValidatorTest {
	...
	public void setUp() {		
		...
		apiValidator.setSensorService(sensorService);
		apiValidator.setSensorTypesService(sensorTypesService);
		apiValidator.setComponentTypesService(componentTypesService);	
		apiValidator.setValidator(validator);
	...
	public void validateNokSensors(){
		...
		ApiValidationResults result = apiValidator.validateSensorsAndComponents(sensors, null,false);
	...
	private List<Sensor> getSensorsWithErrors(){
		...
		String componentId = Component.buildId(providerId,componentName);
		
		Sensor sensor = new Sensor(providerId,componentId, sensor1Id);
		...
		Sensor sensor2 = new Sensor(providerId,componentId, sensor2Id);
	...
	private List<SensorType> getSensorTypes(){
		SensorType[] sensorTypes = { new SensorType(temperatureType)};
