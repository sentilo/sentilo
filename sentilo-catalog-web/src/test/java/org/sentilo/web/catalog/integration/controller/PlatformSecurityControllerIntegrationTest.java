...
import org.sentilo.web.catalog.controller.api.PlatformSecurityController;
...
public class PlatformSecurityControllerIntegrationTest {
	
	@Autowired
	private PlatformSecurityController controller;
	...
	public void getPermissions(){						
		Permissions permissions = controller.getPermissions();				
		assertTrue("Found " + permissions.getPermissions().size() + " and must be greater than 0", permissions.getPermissions().size()>= 1);
	}
	
	@Test
	public void getAlarms(){						
		AlertsOwnersResponseMessage alerts = controller.getAlarms();				
		assertTrue("Found " + alerts.getAlerts().size() + " and must be greater than 0", alerts.getAlerts().size()>= 1);
	}
	
	@Test
	public void getAuthorizations(){
		CredentialsDTO credentials = controller.getAuthorizations();
	...
	public void registerUpdateAndDeleteSensors(){				
		String componentId = Component.buildId(NEW_PROV, MOCK_COMPONENT);
		
		SearchFilter filterByProvider = new SearchFilter();
		filterByProvider.addAndParam("providerId", NEW_PROV);
		
		SearchFilter filterByProviderAndComponent = new SearchFilter();
	...
	public void registerUpdateAndDeleteInvalidSensors(){				
		...
		//Update sensors
		doInvalidSensorsUpdate(filterByProvider, filterByProviderAndComponent, componentId);
	...
	private void doDelete(SearchFilter filterByProvider){
	...
	private void doInsert(SearchFilter filterByProvider, SearchFilter filterByProviderAndComponent){
		CatalogInputMessage inputMessage = getSensorsAndComponentsToInsert();
		controller.registerSensors(inputMessage, NEW_PROV);
		List<Component> components = componentService.search(filterByProvider).getContent();
	...
	private void doSensorsUpdate(SearchFilter filterByProvider, SearchFilter filterByProviderAndComponent, String componentId){
		CatalogInputMessage inputMessage = getSensorsToUpdate();
		...
		Sensor sensor1 = sensorService.find(new Sensor(Sensor.buildId(componentId, MOCK_SENSOR_1)));
		Sensor sensor2 = sensorService.find(new Sensor(Sensor.buildId(componentId, MOCK_SENSOR_2)));
		assertTrue(sensor1!=null && KELVIN_UNIT.equals(sensor1.getUnit()));
		assertTrue(sensor2!=null && CELSIUS_UNIT.equals(sensor2.getUnit()));	
	}
	
	private void doInvalidSensorsUpdate(SearchFilter filterByProvider, SearchFilter filterByProviderAndComponent, String componentId){
		CatalogInputMessage inputMessage = getInvalidSensorsToUpdate();
		CatalogResponseMessage response = controller.updateComponentOrSensors(inputMessage, NEW_PROV);
		assertTrue(CatalogResponseMessage.BAD_REQUEST.equals(response.getCode()));	
	}
	
	private void doComponentsUpdate(SearchFilter filterByProvider, SearchFilter filterByProviderAndComponent, String componentId){
		CatalogInputMessage inputMessage = getComponentsToUpdate();
		controller.updateComponentOrSensors(inputMessage, NEW_PROV);
		List<Component> components = componentService.search(filterByProvider).getContent();
		assertTrue(!CollectionUtils.isEmpty(components));
		assertTrue(components.size() == 1);
	...
	private void doInvalidComponentsUpdate(SearchFilter filterByProvider, SearchFilter filterByProviderAndComponent, String componentId){
		CatalogInputMessage inputMessage = getInvalidComponentsToUpdate();
		CatalogResponseMessage response = controller.updateComponentOrSensors(inputMessage, NEW_PROV);
		assertTrue(CatalogResponseMessage.BAD_REQUEST.equals(response.getCode()));	
	}
	...
	private CatalogInputMessage getSensorsAndComponentsToInsert(){		
		CatalogInputMessage inputMessage = new CatalogInputMessage();
	...
	private CatalogInputMessage getComponentsToUpdate(){
		CatalogInputMessage inputMessage = new CatalogInputMessage();
		CatalogComponent component1 = new CatalogComponent();
		...
		CatalogComponent component2 = new CatalogComponent();
		component2.setComponent(MOCK_COMPONENT_2);
		component2.setComponentDesc("mock description");	
					
		
		List<CatalogComponent> catalogComponents = new ArrayList<CatalogComponent>();
