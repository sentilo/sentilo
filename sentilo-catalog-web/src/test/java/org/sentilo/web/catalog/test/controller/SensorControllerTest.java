...
public class SensorControllerTest {
  ...
  public void newSensorResource() {
    when(userDetailsService.getCatalogUserDetails().isSectorialUser()).thenReturn(true);
    when(messageSource.getMessage(anyString(), any(Object[].class), any(Locale.class))).thenReturn("1,2,3");
    when(providerService.search(any())).thenReturn(new SearchFilterResult<>(Arrays.asList(new Provider())));
