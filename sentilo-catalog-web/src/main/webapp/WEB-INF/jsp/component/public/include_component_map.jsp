...
function selectComponentType(id, name, icon) {
	...
	$('#selectedComponentIcon').attr('src', '${iconPrefix}/pins' + icon + '.png');
    ...
    <form class="form-inline row-fluid">
		...
		<div class="btn-group connecta-icon-group">
			<a href="#" data-toggle="dropdown" class="btn dropdown-toggle">
				&nbsp;&nbsp;&nbsp;
				<img src="${iconPrefix}/pins6.png" id="selectedComponentIcon" />
				<span id="selectedComponentLabel">${allTypesLabel}</span>
				<span class="caret"></span>
			</a>
			<ul class="dropdown-menu" id="componentDropdown">
				<c:forEach var="componentType" items="${componentTypes}">
					<li><a href="#" onclick="selectComponentType('${componentType.id}', '<spring:eval expression="componentType.name" htmlEscape="false" javaScriptEscape="true" />', '${componentType.icon}')"><img src="${iconPrefix}/pins${componentType.icon}.png">${componentType.name}</a></li>
				</c:forEach>
				<li class="divider"></li>
				<li><a href="#" onclick="selectComponentType('', '${allTypesLabel}', '6')"><img src="${iconPrefix}/pins6.png">${allTypesLabel}</a></li>
			</ul>
		</div>
