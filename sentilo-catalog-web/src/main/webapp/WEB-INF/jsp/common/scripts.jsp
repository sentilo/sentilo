...
<spring:url value="/static/js/bootstrap2-toggle.min.js" var="bootstrapToggleJS" />
...
<c:if test="${not empty currentRequestMapping}">
	<c:choose>
		<c:when test="${fn:contains(currentRequestMapping, '/component/map')}">
			...
			<spring:url value="/static/js/slick/slick.min.js" var="slickCarousel" />
			<script type="text/javascript" src="${slickCarousel}"></script>
		</c:when>
	</c:choose>
</c:if>
