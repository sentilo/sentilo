...
<script type="text/javascript">
    ...
    function parsePath(path) {
		let aux = [];
		const precision = 14;
		const aLatlngs = path.getLatLngs();
		aLatlngs.forEach(function (item, index) {        
			aux.push(item.toString(precision));
		});
		
		return aux.join().split('LatLng').join('');
	}

    /**
	 * After page is loaded, initializes location hidden field with the current coordinates, formatted as 
	 * map provider prints them as a string.
	 */
	function initLocationHiddenField(){
    ...
    $(document).ready(function() {
        ...
        $('form#component').submit(function( event ) {                        
			...
			// if them have changed
            if(displayPath && displayPath.getLatLngs()){
                updateLocationHiddenFields(parsePath(displayPath));		
            }
