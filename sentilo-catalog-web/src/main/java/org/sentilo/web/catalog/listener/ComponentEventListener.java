...
public class ComponentEventListener extends AbstractMongoEventListener<Component> {
  ...
  @Value("${sentilo.catalog.component.route.maxSize:20}")
   ...
   * existing one and its timestamp is greater than existing location timestamp. Additionally, new
   * location could be inserted into route list if need be.
   *
   * @param newComponent
   */
  ...
  private void updateLocationIfNeedBe(final Component newComponent) {
    ...
    final Component currentComponent = componentService.findById(newComponent.getId());

    if (currentComponent != null && currentComponent.hasLocation()) {
      ...
      final Long currentLocationTs = currentLocation.getFromTsTime();
  ...
  private void removeDuplicateConsecutiveCoords(final Component component) {
    ...
    if (location.getNumberOfCoordinates() != 1) { // It is a polyline
  ...
  private void updateRouteIfNeedBe(final Component component) {
    ...
    if (component.hasLocation() && component.getLocation().getNumberOfCoordinates() != 1) {
      return;
    }
    ...
    // For each location route point candidate validates if it must be a route member
    if (!CollectionUtils.isEmpty(component.getLocationCandidates())) {
    ...
    // Finally, verifies if component's location must be changes by the newest route point's
    // location
    if (route.last().isPresent()) {
      final RoutePoint lastRp = route.last().get();

      if (!component.hasLocation() || (lastRp.getFromTimeTs() > component.getLocation().getFromTsTime()
          && !lastRp.getLocation().equals(component.getLocation().getCoordinates()[0]))) {
        component.setLocation(new Location(lastRp.getLocation()));
      }
    }
