...
public class UserController extends CrudController<User> {
  ...
  protected List<String> toRow(final User user) {
    ...
    if (!SectorUtils.isSectorList(user)) {
  ...
  protected void doBeforeSearchPage(final HttpServletRequest request, final SearchFilter filter) {
    ...
    // If search is about users in a sector then additional filter should be added to search
    SectorUtils.addFilterBySectorIfNeedBe(filter, User.class);
  ...
  protected void doAfterViewResource(final Model model) {
    ...
    final User user = (User) model.asMap().get(getEntityModelKey());
    model.addAttribute(Constants.SECTORS_ATTR, sectorService.findByIds(user.getSectors()));
  }
