...
public class ProviderServiceImpl extends AbstractBaseServiceImpl<Provider> implements ProviderService {
	...
	protected void doAfterInit() {
		this.entityKeyValidator = customEntityValidator;
		super.doAfterInit();
	}
	...
	@Override
	public String getEntityId(Provider entity) {
		return entity.getId();
	}
	...
	public Provider create(Provider provider) {
		// El identificador se informa por pantalla (es obligatorio). El nombre, en caso de no estar informado, se rellena
		// con el valor del identificador.
		if (!StringUtils.hasText(provider.getName())) {
			provider.setName(provider.getId());
		}

		//Validamos la unicidad del identificador: no puede sexistir otra entidad (app o provider) con el mismo identificador.
	 ...
	 */
	public void delete(Collection<Provider> providers) {
	...
	/*
	 * (non-Javadoc)
	 * @see org.sentilo.web.catalog.service.ProviderService#deleteChildDocuments(org.sentilo.web.catalog.domain.Provider)
	 */
	public void deleteChilds(Provider provider){
		...
		SearchFilter childFilter = new SearchFilter();
		childFilter.addAndParam("providerId", provider.getId());
		Query childQuery = buildQuery(childFilter);
		
		getMongoOps().remove(childQuery, Alarm.class);
