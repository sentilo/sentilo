...
public class UserServiceImpl extends AbstractBaseCrudServiceImpl<User> implements UserService {
  ...
  @Override
  public void addUsersToSector(final String sectorId, final String[] usersIds) {
    // db.user.update({userId:{$in:[]}},$push:{sectors: "sectorId"})
    final Query query = buildQueryForIdInCollection(Arrays.asList(usersIds));
    final Update update = new Update();
    update.push("sectors", sectorId);

    getMongoOps().updateMulti(query, update, User.class);
  }
  ...
  @Override
  public void removeUsersFromSector(final String sectorId, final String[] usersIds) {
    final Query query = buildQueryForIdInCollection(Arrays.asList(usersIds));
    final Update update = new Update();
    update.pull("sectors", sectorId);
