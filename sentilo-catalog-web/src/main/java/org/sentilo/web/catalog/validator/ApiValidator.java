...
public class ApiValidator extends ApiBaseValidator<Sensor> {
  ...
  public ApiValidationResults validateSensorsAndComponents(final List<Sensor> sensors, final List<Component> components,
    ...
    final String[] connectivityTypesList = arrayFromMessages(Constants.CONNECTIVITY_TYPES_KEY);
    final String[] energyTypesList = arrayFromMessages(Constants.ENERGY_TYPES_KEY);
    final String[] geodeticList = arrayFromMessages(Constants.GEODETIC_SYSTEM_TYPES_KEY);
    ...
    if (!CollectionUtils.isEmpty(components)) {
      for (final Component component : components) {
        ...
        validateGeodeticSystem(results, component.getGeodeticSystem(), component.getName(), component, geodeticList);
      }
    }
  ...
  private String[] arrayFromMessages(final String messageKey) {
    final String messagesTypes = messageSource.getMessage(messageKey, null, LocaleContextHolder.getLocale());
    return StringUtils.hasText(messagesTypes) ? messagesTypes.split(SentiloConstants.COMMA_TOKEN_SPLITTER) : new String[] {};
  }
