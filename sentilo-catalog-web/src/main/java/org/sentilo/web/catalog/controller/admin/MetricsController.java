...
public class MetricsController extends CatalogBaseController {
  ...
  public String metrics() {
    // Metrics view is only available either if instance is multi-tenant and user is super admin
    // users, or if instance is simple and user is admin and not sectorial
    final CatalogUserDetails userDetails = userDetailsService.getCatalogUserDetails();
    if (userDetails.isSuperAdminUser() || (!super.isMultitenant() && userDetails.isAdminUser() && !userDetails.isSectorialUser())) {
