...
public abstract class BaseComponentController extends CrudController<Component> {
	...
	@Override
	protected List<String> toRow(Component component) {
	...
	@Override
	protected Component buildNewEntity(String id) {
	 ...
	 * @param componentId
	 */
	protected void addComponentIconTo(Model model, String componentId) {
		Component component = componentService.find(new Component(componentId));
		if (component != null) {
			ComponentType type = componentTypesService.find(new ComponentType(component.getComponentType()));
			if (type != null) {
