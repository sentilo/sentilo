...
public class PublicComponentController extends BaseComponentController {
	...
	public @ResponseBody List<MapComponentDTO> getJSONComponentMap(@RequestParam String componentType, Model model) {
		...
		Map<String, String> icons = getIconMap();
		List<MapComponentDTO> result = new ArrayList<MapComponentDTO>();
		for (Component component : componentService.search(filter).getContent()) {
	 ...
	 */
	private void addComponentSensorsTo(Model model, String componentId) {
		SearchFilter filter = new SearchFilter();
	...
	private void translateAndEscapeSensorType(Sensor sensor) {
		...
		SensorType type = sensorTypesService.find(new SensorType(sensor.getType()));
		if (type != null) {
