...
public abstract class ApiConverter {					
	...
	public static List<Sensor> buildSensorsFromCatalogSensors(ApiConverterContext context){
		List<Sensor> sensors = new ArrayList<Sensor>();
		
		List<CatalogSensor> catalogSensors = context.getMessage().getSensors();
		boolean isUpdateAction = context.isUpdateAction();
		
		if(!CollectionUtils.isEmpty(catalogSensors)){
			for(CatalogSensor catalogSensor: catalogSensors){
				Sensor sensor = (isUpdateAction?buildSensorToUpdate(catalogSensor, context):buildNewSensor(catalogSensor, context));
				sensors.add(sensor);				
			}
		}
	...
	public static List<Component> buildComponentsFromCatalogComponents(ApiConverterContext context){
		List<Component> components = new ArrayList<Component>();
		boolean isUpdateAction = context.isUpdateAction();
		List<? extends CatalogElement> resources = (isUpdateAction?context.getMessage().getComponents():context.getMessage().getSensors());		
				
		if(!CollectionUtils.isEmpty(resources)){
			for(CatalogElement resource: resources){
				Component component = (isUpdateAction?buildComponentToUpdate(resource, context):buildNewComponent(resource, context));
	...
	private static Sensor buildNewSensor(CatalogSensor catalogSensor, ApiConverterContext context){				
		String providerId = context.getProviderId();
		String componentId = Component.buildId(providerId, catalogSensor.getComponent());
	...
	private static Sensor buildSensorToUpdate(CatalogSensor catalogSensor, ApiConverterContext context){				
		String providerId = context.getProviderId();
		Sensor sensor = context.getSensorService().findByName(providerId, catalogSensor.getSensor());
	...
	private static Component buildNewComponent(CatalogElement resource, ApiConverterContext context){		
		...
		String providerId = context.getProviderId();
		String name = catalogSensor.getComponent();
	...
	private static Component buildComponentToUpdate(CatalogElement resource, ApiConverterContext context){		
		CatalogComponent catalogComponent = (CatalogComponent) resource;
		
		Component component = context.getComponentService().findByName(context.getProviderId(), catalogComponent.getComponent());		
