...
public class SecurityUtils {
  ...
  private static boolean _showAdminControls(final ActionType action, final CatalogDocument resource) {
    ...
    if (catalogUser.isSuperAdminUser()) {
      ...
      return (!multitenantEnabled && !catalogUser.isSectorialUser()) || allowAdminUserAdminResource(action, catalogUser, resource);
    } else {
      return false;
    }
  }
  ...
  private static boolean allowAdminUserAdminResource(final ActionType action, final CatalogUserDetails catalogUser, final CatalogDocument resource) {
    ...
    switch (action) {
      case LIST:
      case CREATE:
        allowAdmin = (catalogUser.isSectorialUser() && isSectorialResource) || (!catalogUser.isSectorialUser() && isTenantResource);
        break;
      case READ:
      case EDIT:
        allowAdmin = showAdminControlsWhenAccessAdminUser(catalogUser, resource);
        break;
      default:
        break;
    }
  ...
  private static boolean showAdminControlsWhenAccessAdminUser(final CatalogUserDetails catalogUser, final CatalogDocument resource) {
    boolean show = false;
    final boolean isTenantResource = resource instanceof TenantResource;
    final boolean isTenant = resource instanceof Tenant;

    show = (isTenantResource || isTenant) && TenantUtils.isCurrentTenantResource(resource);

    if (catalogUser.isSectorialUser()) {
      // If user is sectorial then admin controls are displayed only if user can manage resource
      show = show && SectorUtils.canSectorialUserAdminResource(catalogUser, resource);
    }
