...
public class AbstractMapController extends CatalogBaseController {
  ...
  public InfoBoxDTO getLastObservations(@PathVariable final String id, @RequestParam(required = false) final String to,
    ...
    // Finally, compute component extended detail url.
    final ComponentType componentType = componentTypesService.findById(component.getComponentType());
    ComponentUtils.compileExtendedDetailUrl(component, componentType);
  ...
  private SensorDTO toSensorDTO(final Sensor sensor) {
    return new SensorDTO(sensor.getSensorId(), sensor.getType(), sensor.getState().name(), sensor.getDescription(), sensor.getSubstate(),
        sensor.getSubstateDesc(), sensor.getUnit(), sensor.getDataType().name(), sensor.getProviderId());
  }
}
