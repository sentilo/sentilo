...
public class ApiController {
  ...
  private static final String COMPONENT_LOCATION_UPDATE_LOCK = "catalog_update_components_locations";
  ...
  public EntitiesMetadataDTO getEntitiesMetadata() {
    ...
    entitiesMetadata.addEntitiesMetadata(applicationService.findAll());
    entitiesMetadata.addEntitiesMetadata(providerService.findAll());
  ...
  public CatalogResponseMessage updateMobileComponentsLocation(@RequestBody final CatalogInputMessage message) {
    ...
    // message attribute contains a list with N instances of type SensorMessageLocation, each with a
    // location of a sensor's value published at a given time
    try {
      final ApiConverterContext context = new ApiConverterContext(message, sensorService, componentService);
      // update component locations/routes must be coordinated between all catalog instances running
      // in the platform
      lockProvider.getLock(COMPONENT_LOCATION_UPDATE_LOCK).lock();
      final List<Component> components = ApiConverter.addRoutePointsCandidates(context);
      ...
      lockProvider.getLock(COMPONENT_LOCATION_UPDATE_LOCK).unlock();
    }
