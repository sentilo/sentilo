...
public class ActivityServiceImpl extends AbstractBaseServiceImpl<Activity> implements ActivityService {	
	...
	/*
	 * (non-Javadoc)
	 * @see org.sentilo.web.catalog.service.ActivityService#saveCurrentActivity(org.sentilo.web.catalog.domain.Statistics.Events)
	 */
	public void saveCurrentActivity(Events currentEvents){
	...
	public List<Activity> getLastActivityLogs(){
		...
		Pageable pageable = new PageRequest(0, 20, Direction.DESC, "timestamp");
		SearchFilter filter = new SearchFilter(pageable);
		SearchFilterResult<Activity> result = search(filter);
	...
	public void deleteOldActivityLogs(){
		if(lastActivity != null){
			...
			int idToCompare = Integer.parseInt(lastActivity.getId())-30;
			Criteria criteria = Criteria.where("_id").lt(idToCompare);
	...
	private Activity getLastActivity(){
		if(lastActivity == null){
			Pageable pageable = new PageRequest(0, 1, Direction.DESC, "timestamp");
			SearchFilter filter = new SearchFilter(pageable);
			SearchFilterResult<Activity> result = search(filter);
	...
	@Override
	public String getEntityId(Activity entity) {		
		return entity.getId();
	}
}
