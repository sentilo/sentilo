...
public class PlatformSecurityController {
	private final Logger logger = LoggerFactory.getLogger(PlatformSecurityController.class);
	...
	public @ResponseBody CatalogResponseMessage registerSensors(@RequestBody CatalogInputMessage message, @PathVariable String providerId) {		
		...
		try{
			...
			List<Component> components = PlatformMessageConverter.buildComponentsFromCatalogComponents(message.getSensors(), componentService, providerId, false);
			List<Sensor> sensors = PlatformMessageConverter.buildSensorsFromCatalogSensors(message.getSensors(), sensorService, providerId, false);	
						
			ApiValidationResults validationResults = validateSensorsAndComponents(sensors, components);
	...
	public @ResponseBody CatalogResponseMessage updateComponentOrSensors(@RequestBody CatalogInputMessage message, @PathVariable String providerId) {				
		...
		try{
			...
			List<Component> components = PlatformMessageConverter.buildComponentsFromCatalogComponents(message.getComponents(), componentService, providerId, true);
			List<Sensor> sensors = PlatformMessageConverter.buildSensorsFromCatalogSensors(message.getSensors(), sensorService, providerId, true);
	...
	private ApiValidationResults validateSensorsAndComponents(List<Sensor> sensors, List<Component> components){				
