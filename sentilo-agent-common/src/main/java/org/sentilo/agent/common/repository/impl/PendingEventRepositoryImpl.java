...
@Repository
public class PendingEventRepositoryImpl implements PendingEventRepository {

  private static final Logger LOGGER = LoggerFactory.getLogger(PendingEventRepositoryImpl.class);

  private static final String QUEUE_SUFFIX = ".pending.events";
  ...
  public PendingEventRepositoryImpl() {
    super();
    agentName = System.getProperty("sentilo.agent.name");
  }
   ...
   * 
   * @see
   * org.sentilo.agent.common.repository.PendingEventRepository#storePendingEvents(java.util.List)
   */
  public void storePendingEvents(final List<EventMessage> pendingEvents) {
    if (!CollectionUtils.isEmpty(pendingEvents)) {
      final String queueKey = agentName.toLowerCase() + QUEUE_SUFFIX;
  ...
  private String[] buildMembersValues(final List<? extends EventMessage> pendingEvents) {
    ...
    for (final EventMessage eventMessage : pendingEvents) {
      members[i] = converter.marshall(eventMessage);
      i++;
    }
