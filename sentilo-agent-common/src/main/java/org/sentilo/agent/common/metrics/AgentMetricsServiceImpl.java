...
public abstract class AgentMetricsServiceImpl extends SentiloArtifactMetricsServiceImpl implements AgentMetricsCounter {
  ...
  protected Map<String, Object> collectCustomMetrics() {
    ...
    /*
     * if (getRedisPool() != null) { metrics.put(REDIS_POOL_METRICS, collectRedisPoolMetrics()); }
     */
    return metrics;
  }

  /*
   * protected Map<String, Object> collectRedisPoolMetrics() { final Map<String, Object>
   * redisPoolMetrics = new HashMap<String, Object>();
   *
  ...
  protected String getName() {
    return AgentMessagingUtils.getAgentName();
  }
  ...
  /*
   * @SuppressWarnings("unchecked") private Pool<Jedis> getRedisPool() { if (redisConnectionFactory
   * != null && pool == null) { final Field poolField =
   * ReflectionUtils.findField(JedisConnectionFactory.class, "pool");
   * ReflectionUtils.makeAccessible(poolField); pool = (Pool<Jedis>)
   * ReflectionUtils.getField(poolField, redisConnectionFactory); }
   *
   * return pool; }
   */
}
