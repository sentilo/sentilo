...
public class MessageListenerImpl extends AbstractMessageListenerImpl {
  ...
  private void updateLocation(final EventMessage eventMessage) {
    ...
    // timestamp
    logger.debug("update location for event {}", eventMessage);
    ...
    sensorLocation.setFromTsTime(DateUtils.parseTimestamp(eventMessage.getTimestamp()));

    asyncResourceUpdater.addResourceToUpdate(sensorLocation);
  }

  private String buildCacheKey(final EventMessage eventMessage) {
    return eventMessage.getProvider().concat(SentiloConstants.SENTILO_INTERNAL_TOKEN).concat(eventMessage.getSensor())
        .concat(SentiloConstants.SENTILO_INTERNAL_TOKEN).concat(eventMessage.getTimestamp());
  }
