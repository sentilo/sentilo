...
public class ComponentLocationUpdaterTest {
  ...
  public void processEmptyList() {
    ...
    verify(restClient, times(0)).put(anyString(), anyString());
  }
  ...
  public void process() {
    ...
    verify(restClient).put(anyString(), anyString());
  ...
  public void processWithErrors() {
    ...
    doThrow(RESTClientException.class).when(restClient).put(anyString(), anyString());
    ...
    verify(restClient).put(anyString(), anyString());
    Assert.assertTrue(currentUpdatesAwaiting.size() == 0);
    // oldUpdatesAwaiting size must be 1 because all SensorLocationElement have the same location
    Assert.assertTrue(oldUpdatesAwaiting.size() == 1);
  }
